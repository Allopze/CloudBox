...
export default function Landing() {
  ...
  const navItems = useMemo(
    () => [
      { id: 'beneficios', label: t('landing.nav.benefits'), enabled: config.sections.benefits.enabled },
      { id: 'como-funciona', label: t('landing.nav.howItWorks'), enabled: config.sections.howItWorks.enabled },
      { id: 'features', label: t('landing.nav.features'), enabled: config.sections.features.enabled },
      { id: 'comparativa', label: t('landing.nav.comparison'), enabled: config.sections.comparison.enabled },
      { id: 'seguridad', label: t('landing.nav.security'), enabled: config.sections.security.enabled },
      { id: 'github', label: t('landing.nav.github'), enabled: config.sections.github.enabled },
      { id: 'faq', label: t('landing.nav.faq'), enabled: config.sections.faq.enabled },
    ],
    [config.sections, t]
  );
  ...
  return (
    <div className="min-h-screen bg-white text-dark-900 dark:bg-dark-900 dark:text-dark-100">
        ...
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={isDark ? t('landing.theme.light') : t('landing.theme.dark')}
            >
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <div className="lg:col-span-6">
                ...
                <div className="mt-8 grid grid-cols-2 gap-3 max-w-md">
                  {[
                    { icon: Cloud, label: t('landing.hero.modeCloud'), value: config.links.cloudUrl },
                    { icon: CodeLabelIcon, label: t('landing.hero.modeSelfHosted'), value: t('landing.hero.selfHostedValue') },
                  ].map((item) => {
                    const Icon = item.icon as any;
                    return (
                      <div
                        key={item.label}
                        className="rounded-xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-800 px-4 py-3"
                      >
              ...
              <div className="lg:col-span-6">
                {config.assets?.heroImageUrl ? (
                  <div className="rounded-2xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-800 shadow-sm overflow-hidden">
                    <img src={config.assets.heroImageUrl} alt="CloudBox" className="w-full h-auto" />
                  </div>
                ) : (
                  <CloudBoxMockup />
                )}
                {loading && <div className="mt-3 text-xs text-dark-400">{t('landing.loading')}</div>}
              </div>
            </div>
          </div>
        </section>
      ...
      {config.sections.benefits.enabled && (
        <Section id="beneficios" eyebrow={t('landing.section.eyebrow')} title={config.sections.benefits.title}>
      ...
      {config.sections.howItWorks.enabled && (
        <Section id="como-funciona" eyebrow={t('landing.section.eyebrow')} title={config.sections.howItWorks.title}>
      ...
      {config.sections.features.enabled && (
        <Section id="features" eyebrow={t('landing.section.eyebrow')} title={config.sections.features.title}>
          <div className="grid lg:grid-cols-3 gap-4">
            {config.sections.features.groups.map((group) => (
              <div key={group.id} className="card p-6">
                ...
                {group.description && (
                  <div className="mt-1 text-sm text-dark-600 dark:text-dark-300">{group.description}</div>
                )}
                <div className="mt-5 space-y-3">
                  {group.items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="flex gap-3 rounded-xl border border-dark-200 dark:border-dark-700 bg-dark-50 dark:bg-dark-900/40 px-4 py-3"
                      >
          ...
          {config.assets?.featureImageUrl && (
            <div className="mt-10 rounded-2xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-800 overflow-hidden shadow-sm">
              <img src={config.assets.featureImageUrl} alt="CloudBox" className="w-full h-auto" />
            </div>
          )}
        </Section>
      )}
      ...
      {config.sections.comparison.enabled && (
        <Section id="comparativa" eyebrow={t('landing.section.eyebrow')} title={config.sections.comparison.title}>
          <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-dark-200 dark:border-dark-700 bg-white dark:bg-dark-800">
            <div className="grid grid-cols-3 px-5 py-3 text-xs font-medium text-dark-500 dark:text-dark-400 bg-dark-50 dark:bg-dark-900/40">
              <div>{t('landing.comparison.table.feature')}</div>
              <div>{t('landing.comparison.table.cloud')}</div>
              <div>{t('landing.comparison.table.selfHosted')}</div>
            </div>
      ...
      {config.sections.security.enabled && (
        <Section id="seguridad" eyebrow={t('landing.section.eyebrow')} title={config.sections.security.title}>
      ...
      {config.sections.github.enabled && (
        <Section id="github" eyebrow={t('landing.section.eyebrow')} title={config.sections.github.title}>
          <div className="grid lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-5 rounded-2xl border border-dark-200 dark:border-dark-700 bg-dark-50 dark:bg-dark-800 p-6">
              <div className="text-sm font-semibold text-dark-900 dark:text-white">{t('landing.github.requirements')}</div>
      ...
      {config.sections.useCases.enabled && (
        <Section id="casos" eyebrow={t('landing.section.eyebrow')} title={config.sections.useCases.title}>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      {config.sections.faq.enabled && (
        <Section id="faq" eyebrow={t('landing.section.eyebrow')} title={config.sections.faq.title}>
